...
sentence_measures <- function(crowd_data, baseline_data, relations) {
  ...
  for (idx in 1:length(sent_vec[, 1])) {
    ...
    for (jdx in 1:length(relations)) {
      ...
      sent_vec[idx, paste(relations[jdx], "srs", sep = "_")] <- cosine(vec, unit_vec)[[1]]
    }
